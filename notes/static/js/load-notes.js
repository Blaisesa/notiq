...
window.deserializeElement = function deserializeElement(elementData) {
    ...
    switch (type) {
        ...
        case "text":
        case "code":
            contentContainer.innerHTML = `<div contenteditable="true" class="${type === 'code' ? 'code-block' : 'editable-text'}">${content}</div>`;
            break;
        ...
        case "table":
            ...
            let tableHTML = `
                ...
                <table class="simple-table">
                    <thead><tr>${headers.map(h => `<th contenteditable="true">${h}</th>`).join('')}</tr></thead>
                    <tbody>
                        ${rows.map(row => 
        ...
        case "voice":
        case "img-text":
            contentContainer.innerHTML = data.url ? `<img src="${data.url}" alt="Saved Media">` : `<p>Saved ${type} content</p>`;
            break;
    }
...
// --- LOAD NOTE FUNCTION (MUST BE GLOBAL) ---
