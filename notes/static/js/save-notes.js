...
window.serializeCanvas = function () {
    return Array.from(canvas.children).map((el) => {
        ...
        switch (type) {
            ...
            case "text":
            case "code":
                const editable = contentContainer.querySelector('[contenteditable="true"]');
            ...
            case "checklist":
                ...
                contentContainer.querySelectorAll(".checklist-item").forEach((item) => {
                    const textEl = item.querySelector('[contenteditable="true"]');
            ...
            case "table":
                const headers = Array.from(contentContainer.querySelectorAll("thead th")).map(th => th.textContent);
                const rows = Array.from(contentContainer.querySelectorAll("tbody tr")).map(tr =>
            ...
            default:
                const defaultEditable = contentContainer.querySelector('[contenteditable="true"]');
...
window.deleteNote = async function () {
    ...
    if (!confirm(`Delete note ID ${window.currentNoteId}? This cannot be undone.`)) return;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (saveButton) saveButton.addEventListener("click", () => window.saveNote());

    const deleteButton = document.querySelector(".toolbar #delete");
    if (deleteButton) deleteButton.addEventListener("click", () => window.deleteNote());
});
