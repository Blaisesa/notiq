...
window.renderNoteHistory = function renderNoteHistory(notes) {
    ...
    if (notes.length === 0) {
        listContainer.innerHTML = '<li class="no-notes">No saved notes yet.</li>';
        return;
    }
...
window.handleNoteClick = function handleNoteClick(noteId) {
    if (confirm(`Are you sure you want to load note ID ${noteId}? Any unsaved changes will be lost.`)) {
